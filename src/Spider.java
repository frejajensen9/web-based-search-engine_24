/*
 * Spider.java -- Vantricia's version
 * - Add a URL normalization function: normalizeURL
 * - Checked with ChatGPT, basically just ask to add a timeout
 * - spider_result.txt, I got 525 line in total for maxPages = 30
 * - I also tried maxPages = 100 & 500 (for fun), both work
 * 
 * Still don't know why I can't load the stopwords.txt in the normal way
 * so I added a few lines
 * if it is not working for u, just change back to the normal way
 */
...
public class Spider {
    ...
    /**
     * URL Normalization:
     * Normalizes a URL by removing trailing slashes from the path and reconstructing the URL.
    */
    private String normalizeURL(String url) {
        try {
            URL parsed = new URL(url);
            String path = parsed.getPath().replaceAll("/+$", ""); // Remove trailing slashes
            return new URL(parsed.getProtocol(), parsed.getHost(), path).toString();
    ...
    private void indexText(int pageID, String text, HTree invertedIndexBody) throws IOException {
        ...
        Set<String> stopWords = loadStopWords("/stopwords.txt"); // "src/stopwords.txt"
